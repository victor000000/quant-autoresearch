...
- new **bar axes** (priority) — how you sample the clock. Built: `kyle` (price-impact/illiquidity). Untried families:
  liquidity, run-length, spectral.
- new **unsupervised labels** (priority) — what you call "up". Built: `ker` ✓win, `accel`/`sharpe_scan`/`mfe_mae`
  valid but no win yet. Untried: path-shape, reversal-timing, info-theoretic.
