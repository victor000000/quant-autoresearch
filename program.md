...
| IJR | imbalance / `bgm+ker` / cdf_overlay / t.50 | 2.51 | ⚠️ AUDITS-PASS, ONLINE-FAIL 2026-06-12: +2.09 edge, permute total-collapse, regime-split 3.03/1.92, deflated p=0.003 — AND it cross-fund-replicates the DIA mechanism. FINAL 2026-06-12: beta-calibration re-cut collapses it 2.51→0.58 (≈BH+0.15) — the edge is INSEPARABLE from the isotonic plateau knife-edge (plateau-below-thresh keeps it out; episodic jumps trade). Replay-real, not robustly harvestable. **DEMOTED to fragile watchlist**; DIA's cross-fund replication claim correspondingly weakened to 'mechanism shows, does not robustly monetize' on IJR. |
...
- **Equity-index timing (QQQ/SPY/IWM) closed at every door** — action bracket, VIX/VXX, crash sizers, no-forfeit lean, calendar, options-IV. **One crack found 2026-06-12 (user: 'don't give up on QQQ'):** `permclock/bgm+ker/cdf_overlay/t.45` = 1.51 > BH 1.24 — the FIRST QQQ config above hold in project history (the morning's ordinal clock × the regime ensemble; bit-exact, permute-EDGE-collapse pass). Case strengthened same-day: **neighborhood-ROBUST** (t.40 1.45 / t.45 1.51 / t.50-dd 1.27, ALL > BH — the GLD signature, not the crash-lead knife-edge), ONEQ sibling directionally replicates (+0.12 edge, clean permute collapse; QQEW degenerate), decay strengthening, e-value monitored. Beta-calibration artifact test PASSES (1.275 still > BH — signal core real). Online-fail ROOT-CAUSED (forensics 2026-06-12): features all in-distribution; a small (~0.01-0.03) batch-vs-online raw-score drift is amplified by a NEAR-VERTICAL isotonic segment at the operating point (the IJR class, milder) — parity hunt CLOSED 2026-06-12 (5 instrumented runs): features match to FLOAT32 EPSILON (1e-7), kept-order/serialization/tree-counts all bit-verified; the residual is INTRINSIC — the bgm member early-stopped at 5 TREES and a high-traffic hist split sits at a float32 boundary, so 1e-7 op-order noise flips a ~0.03 leaf. **New law: tiny early-stopped ensembles are float-edge DECISION-UNSTABLE — never live-grade.** Re-cuts COMPLETE 2026-06-12: es100 = no-op (predict uses best_iteration regardless of patience); xgb_fixed (200 trees) KILLS the edge (0.87 < BH; val_auc 0.67→0.59, overfit) — **the edge lives IN the 5-tree coarse structure and cannot be made float-stable without destroying it**. Final status: QQQ permclock = REAL, REPLAY-GRADE, STRUCTURALLY NON-DEPLOYABLE in current form; e-process accrues on the replay cell; xgb_bag also fails (1.13 < BH — averaging dilutes the coarse structure). ALL routes exhausted incl. the capacity-DOWN doors (logit 0.64, depth-1 stumps 1.20≈BH+0.03): the conversion-critical signal lives in depth-3 interactions of exactly those 5 trees — **the edge PEAKS at the unstable point** (full spectrum: logit 0.64 < stumps 1.20 ≈ BH < 5-tree 1.51 [unstable] > bag 1.13 > 200-tree 0.87). CONCLUSIVE + CERTIFIED: the deployable online artifact (live_trade, 620d warmup, 503 orders) earns Calmar ~1.23 vs hold ~1.17 over 3y — the online path retains a +0.06 REMNANT of the 1.51 replay edge (float-noise degradation quantified end-to-end). Below the +0.15 gate: not deployment-grade. e-process scores the replay cell; QQQ's deployable optimum remains `always_long`. Still FRAGILE: regime-split 3.69/0.27 FAIL + deflated p=0.26 at N=52 — watchlist not crown; deflation can only relax via window growth (e-process), NOT more trials. `always_long` remains QQQ's deployable optimum meanwhile. SPY/IWM stay fully closed.
