...
**To crown** (human/Opus): decay check on the GLD PCA cell, then set GLD's reduce to `pca` + restate the stale stored 4.02 to the current-window number. ✅ **Restructure regression fixed**: ~21 audit/research scripts computed `results/`/`knowledge.json` via `dirname(dirname(__file__))` → resolved to `scripts/` after the `scripts/{audit,research}/` move (whole honesty toolchain was FileNotFound). Swept to `lb.paths` (commit 2a7ec13). **PR #1 needs the same fix before merge.**
