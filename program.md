...
- **Equity-index timing (QQQ/SPY/IWM) closed at every door** — action bracket, VIX/VXX, crash sizers, no-forfeit lean, calendar, options-IV. **One crack found 2026-06-12 (user: 'don't give up on QQQ'):** `permclock/bgm+ker/cdf_overlay/t.45` = 1.51 > BH 1.24 — the FIRST QQQ config above hold in project history (the morning's ordinal clock × the regime ensemble; bit-exact, permute-EDGE-collapse pass). But FRAGILE: regime-split 3.69/0.27 FAIL, deflated p=0.26 at N=52 — same tier as EWZ/UGL, watchlist not crown. SPY/IWM stay fully closed; `always_long` remains QQQ's deployable optimum pending window growth.
