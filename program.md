...
2. **Joint-KDE quadrant regime as a categorical FEATURE** → `ml_ext.py` / `features.py` (NEXT). TRAIN-fit Gaussian KDE on (return, vol); emit the discretised cell id (+ the conditional return-quantile given the vol bucket) as past-only features; race under `reduce=infogain` on GLD. Distinct from the parametric `regime_gmm` gate — feed the state as a *feature*, not a hard gate (Wang: hand-wired rules are brittle). Higher EV than a sizer: it adds *signal*, not just risk-shaping.
