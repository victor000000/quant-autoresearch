...
**STATE (2026-06-04 late): FULL MODULE SWEEP COMPLETE — single-ticker frontier comprehensively saturated; loop now in MAINTENANCE mode (user choice).** Every pipeline module was tested this session with genuinely-new leak-safe methods, ALL DISCARD: AXES volofvol/wavelet/amihud/ddonset (lose to asset-intrinsic champion clocks); LABELS transfer_entropy_dir (found val_auc 0.58 TLT / 0.64 DBC but untradeable on buy-hold drifters), visgraph (Calmar 3.19 SOXX but val_auc 0.49 = artifact, gate caught it); FEATURE rich-VR (Calmar-negative); REDUCER mRMR (2.62<4.02, redundancy penalty drops useful features — IG's pure relevance optimal); SIZER ddbreaker (3.67<4.02, hard circuit-breaker whipsaws — dd_overlay smooth throttle optimal). GLD champion reproduced **4.0218 bit-exact 3×** through all churn. Two load-bearing truths: (1) the weakest names are BUY-HOLD-OPTIMAL (structural ceiling — timing only sacrifices drift), (2) new methods routinely FIND structure (val_auc>0.6) that doesn't convert to profit (label-relevance≠profit-relevance). See [[new-methods-characterized]].

**DECISION (user, 2026-06-04):** the genuine escape — the **cross-sectional spread path** (trend on a constructed spread A−βB gated on POSITIVE autocorrelation, the no-public-precedent "wall-breaker", `docs/analysis/WORKFLOW_PRIOR_ART.md`) — was OFFERED and **DECLINED**: stay PURE SINGLE-TICKER. So the loop is now MAINTENANCE/MONITORING, NOT method-building: watch book decay (e-value/decay monitors, cached-series reads), re-validate champions on staleness, keep records — do NOT grind new 0-EV single-ticker methods (each adds multiple-testing burden for ~0 edge). Re-open only on a genuinely-new method IDEA or a user decision to relax single-ticker / add an input. Book healthy (2026-06-04 e-value: all 5 members holding, no decay).

**Deep leak hunt (2026-06-04, done) — found + FIXED 2 real LATENT leaks the fast test couldn't catch (both DORMANT, zero deployed blast radius).** 6-locus adversarial hunt, every finding verified EMPIRICALLY (the prior 13-agent audit missed the logdollar leak; code audits miss leaks). The new axes (volofvol/wavelet/amihud) + labelers + the contract verified CLEAN. Two real leaks, neither in any deployed champion:
1. **Embargo was hardcoded `_EMBARGO=200`** but `CONFIG['horizons']` is user-overridable/uncapped → a horizons>200 override bleeds early-test labels into cal.fit/eval_set. Never triggered (no champion uses horizons>200; intraday probes used [10–40]). FIXED: `_EMBARGO = max(200, max(horizons))` (horizon-aware).
2. **`features=termstruct` cross-asset alignment was POSITIONAL** (`spy_lc[:N]`) → primary bar i paired with secondary bar i, which at a different cadence can be a FUTURE minute = look-ahead. Only affects termstruct, which DISCARDED on VIXY/HYG (so the leak made those look BETTER → the "termstruct doesn't help" conclusion is even stronger leak-free). FIXED: timestamp-aware as-of join (`searchsorted` on secondary bar_ts; each primary bar uses the last secondary bar at-or-before it).
